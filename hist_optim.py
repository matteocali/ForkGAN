...
def extract_hist(images: list, b_half: bool = False) -> np.ndarray:
    # Extract all the night iamges Y histograms and perform a mean
    ref_ycrcb_images = [cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb) for img in tqdm(images, desc='Converting to YCrCb color space', leave=False)]
    if b_half:
        ref_y_hist = [cv2.calcHist(img[img.shape[0]//2:, :], [0], None, [128], [0, 256]) for img in tqdm(ref_ycrcb_images, desc='Extracting Y histograms', leave=False)]
    else:
        ref_y_hist = [cv2.calcHist(img, [0], None, [256], [0, 256]) for img in tqdm(ref_ycrcb_images, desc='Extracting Y histograms', leave=False)]
...
def hist_spec(images: list, hist: np.ndarray) -> tp.Tuple[tp.List, tp.List, tp.List]:
    ...
    for img in tqdm(images, desc='Equalizing histograms', leave=False):
    ...
    for img in tqdm(images, desc='Increasing saturation', leave=False):
        ycrcb_img = cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb)  # Convert to YCrCb color space
    ...
    for img in tqdm(images, desc='Specializing histograms', leave=False, total=len):
        ycrcb_img = cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb)  # Convert to YCrCb color space
...
if __name__ == '__main__':
    ...
    ref_y_hist = extract_hist(ref_images, b_half=True)
